...
__device__  uint32_t tau0(uint32_t A);
uint32_t tau(uint32_t A);
uint32_t L_ap(uint32_t B);
uint32_t T_ap(uint32_t Z);
uint32_t *expand_key(bool encryption, byte *key);
__device__ uint32_t L(uint32_t B);
__device__ uint32_t T(uint32_t Z);
__device__ uint32_t F0(const uint32_t *X, uint32_t rk);
__device__ uint32_t F1(const uint32_t *X, uint32_t rk);
__device__ uint32_t F2(const uint32_t *X, uint32_t rk);
__device__ uint32_t F3(const uint32_t *X, uint32_t rk);
void init(bool encryption, byte *keys);
__global__ void process_block(byte *in, byte *out, long max_blocks);
__device__ __host__ uint32_t big_endian_to_int(const byte *bs, uint32_t off);
__device__ void int_to_big_endian(uint32_t n, byte *bs, uint32_t off);
...
__constant__ uint32_t rk[32];

__device__ uint32_t tau0(uint32_t A) {
uint32_t b0 = Sbox0[(A >> 24) & 0xff] & 0xff;
uint32_t b1 = Sbox0[(A >> 16) & 0xff] & 0xff;
uint32_t b2 = Sbox0[(A >> 8) & 0xff] & 0xff;
uint32_t b3 = Sbox0[A & 0xff] & 0xff;
...
void init(bool encryption, byte *keys) {
    ...
    cudaMemcpyToSymbol(rk, RK, sizeof(uint32_t) * 32);
    free(RK);
}

__global__ void process_block(byte *in, byte *out, long max_blocks) {
    long inOff = blockIdx.x * blockDim.x + threadIdx.x;
    if (inOff > max_blocks) {
        return;
    }
    inOff *= BLOCK_SIZE;
...
void encrypt0(byte *plain, byte *encrypted, size_t len, byte *key) {
    ...
    byte *cuda_plain, *cuda_encrypted;
    cuda_ret = cudaMalloc(&cuda_plain, sizeof(byte) * len);
    if (cuda_ret != cudaSuccess) {
        FATAL("Unable to malloc more CUDA memory");
    }
    cuda_ret = cudaMalloc(&cuda_encrypted, sizeof(byte) * len);
    if (cuda_ret != cudaSuccess) {
        FATAL("Unable to malloc more CUDA memory");
    }

    cudaDeviceSynchronize();
    cuda_ret = cudaMemcpy(cuda_plain, plain, sizeof(byte) * len, cudaMemcpyHostToDevice);
    if (cuda_ret != cudaSuccess) {
    ...
    int blocks, threads;
    int n = sizeof(byte) * len / BLOCK_SIZE;
    if (n > 1) {
    ...
    process_block<<<blocks, threads>>>(cuda_plain, cuda_encrypted, n);
    cudaDeviceSynchronize();
    cuda_ret = cudaMemcpy(encrypted, cuda_encrypted, sizeof(byte) * len, cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    if (cuda_ret != cudaSuccess) {
        FATAL("Unable to copy memory to device");
    }
    ...
    cudaFree(cuda_plain);
    cudaFree(cuda_encrypted);
}

void decrypt0(byte *encrypted, byte *decrypted, size_t len, byte *key) {
    cudaError_t cuda_ret;
    byte *cuda_encrypted, *cuda_decrypted;
    cuda_ret = cudaMalloc(&cuda_encrypted, sizeof(byte) * len);
    if (cuda_ret != cudaSuccess) {
        FATAL("Unable to malloc more CUDA memory");
    }
    cuda_ret = cudaMalloc(&cuda_decrypted, sizeof(byte) * len);
    if (cuda_ret != cudaSuccess) {
        FATAL("Unable to malloc more CUDA memory");
    }

    cudaDeviceSynchronize();
    cuda_ret = cudaMemcpy(cuda_encrypted, encrypted, sizeof(byte) * len, cudaMemcpyHostToDevice);
    if (cuda_ret != cudaSuccess) {
        FATAL("Unable to copy memory to device");
    }
    cudaDeviceSynchronize();
    ...
    process_block<<<blocks, threads>>>(cuda_encrypted, cuda_decrypted, n);
    cudaDeviceSynchronize();
    cuda_ret = cudaMemcpy(decrypted, cuda_decrypted, sizeof(byte) * len, cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    if (cuda_ret != cudaSuccess) {
        FATAL("Unable to copy memory to device");
    }
...
bool verify(const char *input_path, const char *decrypted_path) {
    long size1, size2;
    byte *data1 = read_file(input_path, &size1);
    byte *data2 = read_file(decrypted_path, &size2);
